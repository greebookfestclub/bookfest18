= Git のコミットログはソフトウェアアーキテクチャを考えるヒントになりうるか -GQL と jq を用いてコミットログを分析する-
...
ソフトウェア開発における重要な設計原則の一つに、「単一責任の原則（Single Responsibility Principle 以下、SRP）」があります。 SRP は、 Robert C. Martin（通称Uncle Bob）氏によって提唱されており、同氏のブログ記事「The Single Responsibility Principle@<fn>{srp_bob}」で詳細に解説されています。この記事における SRP の核心は、「モジュールは変更するための理由を1つだけ持つべきである」という考え方です。
...
Git でコードベースをバージョン管理していれば、どのファイルがどれだけ変更されているかという情報は、コミットログに記録されています。であるならば、コミットログを分析することを通じて、複数の責務を負っているファイルの候補を見つけ出すことができるはずです。

今回は、このような仮説に基づいて、Git のコミットログから変更頻度の高いファイルを見つけ出す方法を検討しました。

== アプローチ方法
ここからは、実際にどのようにコミットログを分析するか、アプローチについて説明していきます。本記事では、GQL を用いてコミットログの分析を行います。

=== GQL について
Git Query Language(GQL、以下 GQL)@<fn>{gql} は、 Git リポジトリに対して、 SQL ライクなクエリを実行できる問い合わせ言語とその処理系です。 AmrDeveloper 氏によって開発され、 GitHub 上で公開されています。
...
commits, diffs, diffs_changes といったテーブルに対して、標準的な SQL ステートメントである SELECT を用いたデータの抽出や、 WHERE によるフィルタリング、 GROUP BY による集計、 HAVING による集計結果のフィルタリングなどが利用可能です。
...
//footnote[cannot_join][今回は commits テーブルと diffs_changes テーブルとを2回のクエリにわけて取得しています。 GQLは SQL 文における JOIN 句もサポートしているとドキュメントにも記載されています。しかし実際に JOIN を用いてクエリを書いて実行してみると、期待とは異なる出力結果が得られることがあったため、今回は別々のクエリで取得することにしています。]
...
//cmd{
$ gitql -r . -q  "$(cat diffs_changes.gql)" --output json  | jq .
[
  {
...
//cmd{
$ gitql -r . -q  "$(cat diffs_changes.gql)" --output json  | jq "$(cat filter_files.jq)" | jq 'group_by(.path) | map({path: .[0].path, count: length}) | sort_by(-.count)'
[
  {
    "path": "Gemfile.lock",
    "count": 7
...
今回取得したコミットの中では @<tt>{Gemfile.lock} が最も更新されていたようです。これに関してはもっともな結果ではありますが、ロジックが含まれるファイルではなく、今回の趣旨には沿わないのでこのファイルは除外して考えることにしましょう。
その次には、 @<tt>{query_methods.rb} というファイルが最も頻繁に更新されていたようです。以下のコマンドの結果から、変更総数80件のうち3件（約3.8%）がこのファイルに対する変更であるとわかります。
//cmd{
...
GQL というツールは、前述したJOIN句やIN句の挙動など、SQLとの完全な互換性がない点があり、やや痒いところに手が届かないと感じる部分もありますが、 SQL についての多少の知識さえあれば直感的に扱えるツールであるという印象を持ちました。
今回はファイルの変更頻度について着目しましたが、 Git のコミットログにはそれ以外にもたくさんの情報が含まれており、別の観点では「コミットログから誰がどの領域についての知識を多く持っているか」という視点で分析するなど、業務で普段メンテナンスしているコードベースに対して分析を行えば、活用できる方法はたくさんありそうです。
