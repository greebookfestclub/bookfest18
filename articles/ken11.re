...
もはや世界はAIに対して「付加価値」ではなく「当然のもの」として位置づけはじめていて、@<b>{「AIオリエンテッド」な時代}が訪れようとしています。AIを機能として取り入れることを前提としたサービス設計が求められ、同時に、AIを使うことで発生する問題に対して初期段階からよくよく考えておく必要が生じています。
...
大手のAIサービスは各社が「nトークンあたりいくら」というコスト体系を採用しています。例えば、Claude 3.5はAmazon Bedrockを使った東京リージョンで1000トークンの入力で0.003ドル、出力で0.015ドル、バッチ処理ならそれぞれその半額となっています。@<fn>{bedrock-cost}
...
一方で、独自モデルを常時ホストするとなると固定費として月に数万〜数百万円がかかってくることになります。例えば、Llama 2の7Bパラメータモデルがギリギリ動作するAWS Inferentiaの@<tt>{Inf2.xlarge}のインスタンスは、1時間あたり1.1373ドルの費用がかかります@<fn>{inf2-cost}。
...
偉い人は「AI活用により開発生産性を遙かに高める」と言いますが、現実はそんなに甘くはありません。前述の通りAIは誤った実装を出してくることがあるますし、そもそもAIに的確に指示を出すというのはより多くの日本語を書かないといけなくて面倒なことが多いのです。
...
いま主流のAIはいずれもこのTransformerをベースにしており、積和演算のためのコストが非常に高くなっています。AGIにたどり着くには、Transformerの次のアーキテクチャが出現するか、あるいはTransformerの計算を奇跡的な低コストで実現できるハードウェアが出てこないと無理だろうと考えています。
