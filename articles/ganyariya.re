...
この章では、@<kw>{業務駆動開発}についての個人的ふりかえりと今後の展望をお話しします。

業務駆動開発が自分にもあいそう、という方がもしみなさんのなかにもいらっしゃればぜひお試しください。
 ...
 * プロダクトで使っているツールについて、使い方は知っているもののそれ自体の構築方法や仕組みはわかっていない
 * プロダクト内で発生している課題を解決できる技術の存在は知っているが、その技術を触ったことがなく工数の検討がつかない
...
枯れた技術や設計（DDD / CleanArchitecture）の勉強を優先しており、これらは当然業務でも役立っています。
...
本当であれば、業務中にわからないことを丁寧に調べて理解しながら応用する、ができれば一番よいです。
しかし、プロダクトを運用しているとなかなかにタスクが多く、わからないことを丁寧に調べる、という時間はとれません。

また、タスクの割り振りをする際にチームメンバのアサイン状況が圧迫していると、工数を抑えるために該当の技術について経験がある方にタスクを振ることが多いと思います。これでは属人化が発生し、新たに学びたい人に機会が与えられづらいです。
...
ここで、 それらの使い方はなんとなくわかっているものの、それぞれの構築方法は知らなかったため自分で組んでみることにしました @<fn>{ganyariya-house-k8s}。
...
とくに Kubernetes については、プロダクトにおける PVC の設定を変更して費用を削減できたり、 より安定するような VM の配置方法に変更できました。

個人で基礎を学び、それを業務で応用する、というケースがきちんと達成できたと考えています。

=== Jenkins

業務において Jenkins をビルドパイプラインとして利用しています。
構築方法や動作原理が同様にわからなかったため、こちらも自分で構築しました。
...
この作業によって、どう Jenkins マスタとワーカーノードがプロダクトで動いており、かつ永続化と CasC の設定が行われているかが理解できるようになりました。

別件として、Jenkins のワーカーノードをより安定化させたいという要件がプロダクトで発生しました。
ワーカーノードとして利用している Google Cloud の VM は、社内内製ツールで性能や台数が設定されています。
ワーカーノードの挙動を安定化させるにはこの内製ツールを実現させている技術を理解する必要があるため、業務駆動開発で先に学習しておくことにしました。
...
ここで学習した内容をもとに、プロダクトの Google Cloud VM の設定値を変更しました。
その結果、ワーカーノードがより安定して稼働するようになり、無事要件を達成できました。

=== o11y

開発環境 API トレースを OpenTelemetry で取得して Grafana で可視化したい、というプロダクトの要件がありました。
これを実現するために、業務駆動開発で o11y、 とくに OpenTelemetry を学習しました。

手始めに簡単な Go ウェブアプリに o11y ツールを導入することから始めました @<fn>{ganyariya-go-o11y}。
OpenTelemetry Collector を経由して Grafana Tempo にトレースを送り Grafana で可視化できるようにする、という流れを経験できました。

くわえて、Grafana と Loki についてより詳しくなるために helm-stack を活用してミニ PC k8s をモニタリングできるようにしました @<fn>{ganyariya-house-k8s}。

これらの経験をもとに、プロダクトで OpenTelemetry を実際に導入し、開発環境のトレースを Grafana 上で確認できるようにしました。
...
また、個人の環境のため好き勝手に作業して壊してよいところも大きいです。業務で破壊してしまうとサービスや開発環境に影響を与えてしまうためなかなか挑戦しづらいです。
