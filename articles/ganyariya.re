...
//caution{
    @<kw>{業務駆動開発} は ganyariya の造語であることに注意してください。
//}
...
業務駆動開発とは、@<tt>{業務でわからなかったこと・これから使いそうなこと} を個人開発でなんとなくわかる段階まで持っていく駆動法です。なんとなくわかる段階まで持っていったらあとは業務内で学び応用しつつ伸ばしていきます。

たとえば、お仕事をしていると以下のような状況に出くわす事があると思います。

 * プロダクトで使っているツールについて、使い方は知っているがそのツール自体の構築方法や仕組みがわかっていない
 * プロダクト内で発生している課題を解決できる技術があるが、その技術を触ったことがない

1 つのプロダクトを成り立たせるには数多くの技術が必要であり、当然わからないことはたくさんあります。
このわからないことを個人の範囲でキャッチアップし、それを業務に応用していくのが業務駆動開発です。

== どうして業務駆動開発を始めたのか

@<tt>{業務駆動開発} の用語説明を見たみなさんにとって、このような取り組みはエンジニアとして当たり前だろうとお思いになったはずです。自分も今ではそう思います。
...
また、 ganyariya は興味の移り変わりが早いため、いろいろなものを並行した結果すべて中途半端になってしまうことも多くありました。
...
また、タスクの割り振りをする際、チームメンバのアサイン状況が圧迫していると、工数を抑えるために該当の技術や経験がすでにある方にタスクを振ることが多いと思います。

一方で、ある程度その技術に知識があれば工数も見積もりやすく、タスクも振りやすくなります。
...
業務において、 Kuberntes + ArgoCD をつかってバックエンド API の提供を行っています @<fn>{wfs-k8s-argocd}。
...
これだけだと k8s 環境に ArgoCD を載せただけになってしまっているため、 Misskey を k8s 上で構築しました @<fn>{ganyariya-k8s-misskey}。
Cloudflare Tunnels を使用することで家の外からでも Misskey にアクセスできます。

これらの取り組みによって、 Kubernetes と ArgoCD についてより理解を深められました。
とくに Kubernetes については、PVC の設定を変更して費用を削減したり、 開発サーバがより安定するようにVM の配置方法を変更できました。

=== Jenkins

業務において Jenkins をビルドパイプラインとして利用しています。
Jenkins の使い方はわかっているものの、どう構築するのかは k8s と同様にわからなかったため、自分で構築してみることにしました。

ミニ PC の k8s 環境に Jenkins をセットアップし、プロダクトと同様に Jenkins CasC で IaC 管理しました。
Jenkins を起動する際に自動でプラグインをインストールし、k8s 上の pod を worker として利用できるようにしています。
...
手始めに簡単な Go のウェブアプリに o11y ツールを導入することから始めました @<fn>{ganyariya-go-o11y}。
OpenTelemetry Collector を経由して Grafana Tempo にトレースを送り、それを Grafana で可視化できるようにする、という流れを経験できました。

Grafana, Loki についてより詳しくなるために helm-stack を活用してミニ PC k8s をモニタリングできるようにしました @<fn>{ganyariya-house-k8s}。
ArgoCD における Helm Chart の使い方や Grafana, Loki の意義や使い方を知ることができました。

この経験をもとに、プロダクト内で OpenTelemetry を導入し、開発環境のトレースを Grafana 上で確認できるようにしました。
とはいってもトレースを取るための計装コード生成は先輩の方が作ったコードを利用していますし、 Grafana に送信するための Grafana Alloy はインフラの方にご用意いただいています。
...
個人の範囲で入口を学ぶため、それを応用して個人ポートフォリオまで持っていくのは難しいです。

とはいっても、これは業務駆動開発にかかわらず、個人でポートフォリオや OSS を開発するのがモチベーションや時間の観点から難しいというのが原因なのもあります。

== 課題に対する解決策としてアウトプット駆動も取り入れる

業務駆動開発によって、個人で学び業務で応用する、というよいサイクルを回せました。
しかし、個人のエンジニアの視点からすると、個人では入口のみ学ぶため、アウトプットが出せずポートフォリオが増えないという課題が残りました。

そのため、今後は業務駆動開発に並行して、アウトプット駆動開発も取り入れようと思います。
個人で学び業務で応用しつつ、それを個人開発でどんどんアウトプットしてリリースする、というフローを目指そうと思います。
